    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.mahasiswas.update", [$mahasiswa->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_lengkap') ? 'is-invalid' : '' }}" type="text" name="nama_lengkap" id="nama_lengkap" value="{{ old('nama_lengkap', $mahasiswa->nama_lengkap) }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Mahasiswa::JENIS_KELAMIN_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('jenis_kelamin') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="jenis_kelamin_{{ $key }}" name="jenis_kelamin" value="{{ $key }}" {{ old('jenis_kelamin', $mahasiswa->jenis_kelamin) === (string) $key ? 'checked' : '' }} required>
