...
<body>
    <nav class="navbar navbar-expand-lg">
        <div class="container">
            <a class="navbar-brand" href="#">SISPRES</a>
    ...
    <div class="container">
        <h3 class="display-6">Daftar Lomba</h3>
        <div class="owl-carousel owl-theme">
            @foreach ($beritas as $berita)
                <div class="card" style="width: 18rem;">
                    @if ($berita->featured_image)
                        <img class="card-img-top" src="{{ $berita->featured_image->getUrl() }}"
                            alt="Card image cap">
                    @endif
                    <div class="card-body">
                        <h5 class="card-title">{{ $berita->title }}</h5>
                        <p class="card-text">{!! Str::limit(strip_tags($berita->page_text), 100) !!}</p>
                        <a href="{{ route('home.show', $berita->title) }}" class="btn btn-primary">Lebih Lanjut</a>
                    </div>
                </div>
            @endforeach
        </div>
